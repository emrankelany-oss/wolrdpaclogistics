...
function identifyArticleSections(pages, configuredRoots) {
  ...
  for (var j = 0; j < parentRoutes.length; j++) {
    ...
    // If articleRoots is configured, only include sections that match configured roots
    var hasConfiguredRoots = configuredRoots && configuredRoots.length > 0;
    if (hasConfiguredRoots) {
      if (isConfigured && children.length >= 1) {
        sections.push({
